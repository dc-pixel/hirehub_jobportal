...
export default function Home() {
  ...
  return <main>
    ...
    <section className="hero"><div className="container"><div className="eyebrow">{session?`SIGNED IN AS ${session.role.toUpperCase()}`:'SMARTER JOB SEARCH'}</div><h1>Find a job you’ll love.</h1><p>Discover opportunities from growing companies and build the career you want with HireHub.</p><div className="searchbox"><input className="field" aria-label="Job title, skill or company" placeholder="Job title, skill or company" value={query} onChange={e=>setQuery(e.target.value)}/><input className="field" aria-label="Location" placeholder="Location" value={location} onChange={e=>setLocation(e.target.value)}/><select className="field" aria-label="Job category" value={category} onChange={e=>setCategory(e.target.value)}><option value="">All categories</option><option>Development</option><option>QA</option><option>Design</option><option>Marketing</option><option>Sales</option></select><button className="btn primary" onClick={()=>document.getElementById('jobs')?.scrollIntoView({behavior:'smooth'})}>Search Jobs</button></div></div></section>
